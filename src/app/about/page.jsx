  ...
  return (
    <div ref={ref} className="flex flex-col gap-2.5">
      <div className="flex items-center justify-between px-1">
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-subheading">{name}</span>
        <motion.span
          initial={{ opacity: 0 }}
          animate={inView ? { opacity: 1 } : {}}
          transition={{ delay: delay + 0.6 }}
          className="text-xs font-black text-heading"
        >
          {pct}%
        </motion.span>
      </div>
      <div className="h-2 w-full rounded-full bg-card-alt border border-card-border overflow-hidden shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
        <motion.div
          initial={{ width: 0 }}
...
function StatCard({ value, label, icon: Icon, index }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-4 rounded-5xl border border-card-border bg-card px-6 py-10 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] transition-all duration-300 hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.08)]"
    >
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-card-alt text-orange-500 shadow-sm border border-card-border">
        <Icon size={20} strokeWidth={1.5} />
      </div>
      <div className="flex flex-col items-center">
        <span className="text-5xl font-black tracking-tight text-heading leading-none">{value}</span>
        <span className="mt-3 text-center text-[10px] font-black uppercase tracking-[0.2em] text-subheading">{label}</span>
      </div>
    </motion.div>
  )
}
...
function TestimonialCard({ t, delay }) {
  return (
    <motion.div
      {...fadeUp(delay)}
      whileHover={{ y: -8, transition: { duration: 0.3 } }}
      className="group relative flex flex-col justify-between overflow-hidden rounded-4xl border border-card-border bg-card p-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] transition-all duration-300 hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.08)]"
    >
      ...
      <div className="relative z-10 mt-8 flex items-center gap-4">
        ...
        <div>
          <p className="text-sm font-black tracking-tight text-heading">{t.name}</p>
          <p className="text-[11px] font-bold uppercase tracking-widest text-subheading">{t.role}</p>
        </div>
      </div>
    </motion.div>
  )
    ...
    <section ref={containerRef} className="relative w-full overflow-hidden px-5 pb-32 pt-24 md:px-10">
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <div className="mb-12 flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
          <div className="space-y-4">
            ...
            <div className="overflow-hidden">
              <motion.h1
                ...
                className="text-[12vw] font-black leading-none tracking-tighter text-heading md:text-[8vw]"
              >
                The Story.
              </motion.h1>
            </div>
          ...
          >
            <div className="flex items-center justify-end gap-3 text-xs font-black uppercase tracking-widest text-subheading">
        ...
        <div className="relative mb-24 h-px w-full bg-card-border">
          <motion.div
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
        ...
        <div className="mb-28 flex flex-col items-start gap-16 md:flex-row">
          ...
          <div className="flex flex-1 flex-col justify-center">
            ...
            <motion.h2 {...fadeUp(0.1)} className="mb-10 text-4xl font-black leading-tight tracking-tight text-heading md:text-5xl">
              Capturing moments that <br /> feel like memories.
            </motion.h2>

            <div className="space-y-6">
              <motion.p {...fadeUp(0.2)} className="text-lg font-medium leading-relaxed text-paragraph">
              ...
              <motion.p {...fadeUp(0.3)} className="text-sm leading-relaxed text-subheading max-w-lg">
        ...
        <div className="mb-16">
          <motion.div {...fadeUp(0)} className="mb-12 flex flex-col items-start gap-4 md:flex-row md:items-end md:justify-between">
            <h3 className="text-3xl font-black tracking-tight text-heading md:text-5xl">Kind Words.</h3>
